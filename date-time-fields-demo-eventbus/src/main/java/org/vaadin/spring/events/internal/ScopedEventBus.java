...
public abstract class ScopedEventBus implements EventBus, Serializable {
    ...
    private EventBusListener<Object> parentListener = new EventBusListener<Object>() {
        ...
        public void onEvent(final Event<Object> event) {
            log.debug(String.format("Propagating event [{}] from parent event bus [{}] to event bus [{}]", event, parentEventBus,
                    ScopedEventBus.this));
    ...
    public ScopedEventBus(EventScope scope, EventBus parentEventBus) {
        ...
        if (parentEventBus != null) {
            if (AopUtils.isJdkDynamicProxy(parentEventBus)) {
                log.debug(String.format("Parent event bus [{}] is proxied, trying to get the real EventBus instance",
                        parentEventBus));
                try {
                    this.parentEventBus = (EventBus) ((Advised) parentEventBus).getTargetSource().getTarget();
                } catch (Exception e) {
                    log.error("Could not get target EventBus from proxy", e);
                    throw new RuntimeException("Could not get parent event bus", e);
                }
            }
            log.debug(String.format("Using parent event bus [{}]", this.parentEventBus));
    ...
    @PreDestroy
    void destroy() {
        log.trace(String.format("Destroying event bus [{}] and removing all listeners", this));
        listeners.clear();
        if (parentEventBus != null) {
    ...
    public <T> void publish(String topic, Object sender, T payload) {
        log.debug(String.format("Publishing payload [{}] from sender [{}] on event bus [{}] in topic  [{}]", payload, sender, this,
                topic));
    ...
    public <T> void publish(EventScope scope, String topic, Object sender, T payload)
        ...
        log.debug(String.format("Trying to publish payload [{}] from sender [{}] using scope [{}] on event bus [{}] in topic [{}]",
        ...
        if (eventScope.equals(scope)) {
            ...
            log.warn(String.format("Could not publish payload with scope [{}] on event bus [{}]", scope, this));
    ...
    public <T> void subscribe(EventBusListener<T> listener, String topic) {
        log.trace(String.format("Subscribing listener [{}] to event bus [{}]", listener, this));
    ...
    public <T> void subscribe(EventBusListener<T> listener, boolean includingPropagatingEvents) {
        log.trace(String.format("Subscribing listener [{}] to event bus [{}], includingPropagatingEvents = {}", listener, this,
    ...
    public <T> void subscribeWithWeakReference(EventBusListener<T> listener, String topic) {
        log.trace(String.format("Subscribing listener [{}] to event bus [{}] with weak reference",
                listener, this));
    ...
    public <T> void subscribeWithWeakReference(EventBusListener<T> listener, boolean includingPropagatingEvents) {
        log.trace(String.format("Subscribing listener [{}] to event bus [{}] with weak reference, includingPropagatingEvents = {}",
    ...
    private void subscribe(final Object listener, final String topic, final boolean includingPropagatingEvents,
        ...
        log.trace(String.format("Subscribing listener [{}] to event bus [{}], includingPropagatingEvents = {}, weakReference = {}",
                ...
                for (final Method m : clazz.getDeclaredMethods()) {
                    if (m.isAnnotationPresent(EventBusListenerMethod.class)) {
                        if (m.getParameterTypes().length == 1) {
                            log.trace(String.format("Found listener method [{}] in listener [{}]", m.getName(), listener));
        ...
        if (foundMethods[0] == 0) {
            log.warn(String.format("Listener [{}] did not contain a single listener method!", listener));
        }
    }
    ...
    public void unsubscribe(final Object listener) {
        log.trace(String.format("Unsubscribing listener [{}] from event bus [{}]", listener, this));
